...
def register_user(request):
    ...
    if User.objects.filter(username=data.get('username')).exists():
        return Response({"status":"error","message": "Username already Taken or Exist."}, status=status.HTTP_400_BAD_REQUEST)
...
def login_user(request):
    ...
    if user:
        ...
        return Response({"status":"Login Successfull","Access Token": str(refresh.access_token), "user": user_data,})
...
def edit_comment(request):
    ...
    if not request.data.get('comment_id') or not request.data.get('comment_text'):
        return Response({"status":"error", "message": "comment_id and comment_text required"}, status=status.HTTP_400_BAD_REQUEST)
